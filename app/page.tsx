...
export default function Home() {
  ...
  // --- [신규] 시트 데이터를 활용한 동적 캐로슬 상태 ---
  const [bannerIndex, setBannerIndex] = useState(0);
  const [isTransitioning, setIsTransitioning] = useState(true);

  // 구글 시트 데이터 로드 및 뒤로가기 세팅
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    }, 2000); // 사용자가 이미지를 볼 수 있도록 2초로 여유 있게 늘림
  ...
  // 공통 사용 푸터 (Footer) 컴포넌트
  const Footer = () => (
    <footer className="bg-slate-100 text-slate-500 py-10 px-4 mt-auto border-t border-slate-200">
  ...
  if (selectedProductId && currentProduct) {
    ...
    return (
      <main className="min-h-screen bg-slate-50 pb-20 flex flex-col">
        ...
        <div className="max-w-4xl mx-auto bg-white mb-2 shadow-sm w-full">
        ...
        <div className="max-w-3xl mx-auto px-4 bg-white py-10 shadow-sm w-full mb-10">
  ...
  return (
    <main className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <section className="max-w-5xl mx-auto py-10 px-4 flex-grow w-full">
        {loading ? (
